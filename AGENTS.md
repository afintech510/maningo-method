...
- **Purchasing kill switch lives in the DB, not env.** `studio_settings.purchases_enabled` (migration `044`) is the master switch for selling new credits — card packs, cash packs, promos, and gift packs. Flip it at **Admin → Sales → Studio settings**; it takes effect within ~30s (`getStudioSettings` cache TTL) with **no rebuild and no redeploy**. Don't confuse it with `NEXT_PUBLIC_STRIPE_ENABLED`, which only switches card→cash and still sells. Every money route calls `purchasesClosedGuard()` from `src/lib/purchases.ts`, so any new checkout route must call it too. Turning it off does **not** affect booking with credits already held, redeeming gift codes already bought, waitlists, or admin credit adjustments. Member-facing copy is the single constant `PURCHASES_CLOSED_MESSAGE` in that file.
