...
- **Purchasing switches live in the DB, not env.** Three knobs on `studio_settings`, all of which must allow a sale: `purchases_enabled` (master kill switch, migration `044`), `sellable_pack_types` (which packs specifically, `045`), and `gift_purchases_enabled` (gift cards are prepaid credits with no expiry, so they switch separately, `045`). Set them at **Admin → Sales → Studio settings**; changes take effect within ~30s (`getStudioSettings` cache TTL) with **no rebuild and no redeploy**. Don't confuse any of this with `NEXT_PUBLIC_STRIPE_ENABLED`, which only switches card→cash and still sells.
  - Read state via `getPurchaseAvailability()` in `src/lib/purchases.ts`; guard money routes with `packClosedGuard(packType)` or `giftClosedGuard()`. **Any new checkout route must call one of them** — the UI hiding a button is not enough, since checkout URLs are bookmarked and guessable.
  - None of these switches affect booking with credits already held, redeeming gift codes already bought, waitlists, or admin credit grants.
  - A pack that's off keeps its price on the site but stops being a buy button, by card and by cash. Member-facing copy is the single constant `PURCHASES_CLOSED_MESSAGE`.
